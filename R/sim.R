...
#' @param sigma_E SD of spatiotemporal process (Epsilon). Can be a scalar or vector of length time_steps
...
sdmTMB_sim <- function(mesh,
  ...
  if(length(sigma_E) %in% c(1, time_steps)==FALSE) {
    stop("Error: sigma_E needs to be a scalar of vector of length time_steps")
  }
  ...
  if (time_steps > 1L && sigma_E_zero==FALSE) {
    for (i in seq_len(time_steps)) {
      if (i == 1 || rho == 0) {
        epsilon_st[[i]] <-
          rspde2(coords, sigma = sigma_E[1], range = range, mesh = mesh, seed = seed * i)
